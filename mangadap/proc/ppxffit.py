...
#        self.npad = fftpack.next_fast_len(self.templates.shape[0])
        ...
        if reddening_func is None:
            # new version of ppxf 9.2.2 replaces reddening_cal100
            # with the attenuation value instead of E(B-V)
    ...
    def construct(self, kin, tplwgts, addpoly=None, multpoly=None, reddening=None,
        ...
        Args:
            ...
            multpoly (numpy.ndarray): (**Optional**) Coefficients of the
                ...
                are provided.
            reddening (float): (**Optional**) Attenuation A_V value for the
                continuum fit.  Exception is raised if value is expected
                (self.reddening is not None), but none is provided. Changed
                from E(B-V) value in new ppxf version 9.2.2.
            gas_reddening (float): (**Optional**) Attenuation A_V value for the
                gas components.  Exception is raised if value is
                expected (self.reddening is not None), but none is
                provided. Changed from E(B-V) value in new ppxf version 9.2.2.
        ...
        if self.mdegree > 0:
            ...
            mpoly = None if reddening is None else self.reddening_func(self.lam, reddening*default_calzetti_rv())
        gas_mpoly = None if gas_reddening is None else self.reddening_func(self.lam, gas_reddening*default_calzetti_rv())
